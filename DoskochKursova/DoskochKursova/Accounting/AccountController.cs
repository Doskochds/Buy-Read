    ...
    public class AccountController : ControllerBase
    {
        ...
        public async Task<IActionResult> Register([FromBody] RegisterDto model)
        {
            ...
            var token = await _userManager.GenerateEmailConfirmationTokenAsync(user);
                        var callbackUrl = $"https://посилання кинути після створення фронта{user.Id}&token={Uri.EscapeDataString(token)}";
        ...
        public async Task<IActionResult> ConfirmEmail([FromQuery] int userId, [FromQuery] string token)
        {
            ...
            if (result.Succeeded)
            {
                ...
                // return Redirect("силка на фронт, не забути");
        ...
        public async Task<IActionResult> ForgotPassword([FromBody] ForgotPasswordDto model)
        {
            ...
            var callbackUrl = $"фронтенд={user.Email}&token={Uri.EscapeDataString(token)}";
